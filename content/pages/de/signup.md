---
title: Registrieren
description: Erstellen Sie ein kostenloses Konto mit Google, um Ihre Visa-Tage-Berechnungen zu speichern und jederzeit darauf zuzugreifen.
type: Page
noHeader: true
sections:
  - type: Signup
    pageType: signup
    title: Willkommen beim Visa-Tage-Rechner!
    subtitle: Registrieren Sie sich hier, um fortzufahren
    sideImageUrl: https://images.unsplash.com/photo-1524850301259-7729d41d11d9?q=80&w=794&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D
    buttonText: Mit Google registrieren
    errorText: Registrierung mit Google fehlgeschlagen. Bitte versuchen Sie es später erneut.
    buttonErrorText: Bitte stimmen Sie den Nutzungsbedingungen zu
    termsText: Mit dem Fortfahren stimme ich den
    termsLinkText: Nutzungsbedingungen
    termsLink: /terms
---
