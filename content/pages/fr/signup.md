---
title: Inscription
description: Créez un compte gratuit pour enregistrer et accéder à vos calculs de jours de visa à tout moment.
type: Page
noHeader: true
sections:
  - type: Signup
    pageType: signup
    title: Bienvenue sur le Calculateur de jours de visa !
    subtitle: Inscrivez-vous ici pour continuer
    sideImageUrl: https://images.unsplash.com/photo-1524850301259-7729d41d11d9?q=80&w=794&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D
    buttonText: S'inscrire avec Google
    errorText: Échec de l'inscription avec Google. Veuillez réessayer plus tard.
    buttonErrorText: Veuillez accepter les conditions d'utilisation
    termsText: En continuant, j'accepte les
    termsLinkText: conditions générales
    termsLink: /terms
---
